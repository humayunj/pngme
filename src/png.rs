...
use crate::{chunk::Chunk, utils, Error};
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> crate::Result<Chunk> {
        let index = self
            .chunks
            .iter()
            .position(|i| i.chunk_type().to_string().eq(chunk_type));
        if index.is_none() {
            return Err(Error::from("Chunk type doesn't exists"));
        }
        return Ok(self.chunks.remove(index.unwrap()));
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks
            .iter()
            .find(|i| i.chunk_type().to_string().eq(chunk_type))
    }

    pub fn as_bytes(&self) -> Vec<u8> {
        let mut b = Vec::new();
        b.extend_from_slice(&Png::STANDARD_HEADER[..]);
        self.chunks
            .iter()
            .for_each(|c| b.extend_from_slice(&c.as_bytes()[..]));
        b
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        match &bytes[0..8].cmp(&Png::STANDARD_HEADER[..]) {
            Ordering::Equal => (),
            _ => return Err(Error::from("Not a png header")),
        }
        let mut cursor: usize = 8; // skip header
        let mut png = Png { chunks: Vec::new() };
        loop {
            let bytes_left: i32 = bytes.len() as i32 - cursor as i32 - 4 - 4; // first 4 for length and second for crc

            if bytes_left < 4 {
                break;
            }

            let len = utils::u8_slice_to_u32(&bytes[cursor..(cursor + 4)]) as usize;

            if len > bytes_left as usize {
                return Err(Error::from("insufficient chunks data length"));
            }

            let chunk_data = &bytes[cursor..(cursor + len + 12)]; //chunk-length+chunk-type+crc
            let res = Chunk::try_from(chunk_data)?;
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "   Chunks Len: {}", self.chunks.len()).unwrap();
        Ok(())
    }
}
